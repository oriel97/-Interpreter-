...
/*    string fileName = "fly.txt";
    Lexer lexer1(fileName);
    lexer1.parser();
    // print to check that the vector is right
    vector<string> myVectorList = lexer1.getVectorList();
    map<string, Command> CommandMap;
    int index = 0;
    while (index <= myVectorList.size()) {
        Command &c = CommandMap.at(myVectorList[index]);
        if (&c != NULL) {
            index += c.execute(myVectorList, index);
        }
    }*//*
*/
/*
    openDataServerCommand *c = new openDataServerCommand ;
...
int main() {
    ...
    try {
        // 7
        i4->setVariables("x2=2.5;y=#");//error
    ...
    try {
        e9 = i6->interpret("0.12389");
        std::cout << "9: " << e9->calculate() << std::endl;// -3
        delete e9;
        delete i6;
    } catch (const char* e) {
        if (e9 != nullptr) {
    ...
    try {
        // 11
        i8->setVariables("roll=70;y=-4");
        e11 = i8->interpret("-roll/70"); //error
